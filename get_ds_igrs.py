...
def get_ds_igrs(genbank_path, outpath, min_igr_length=50):
    ...
    if gene_list[0][1] > 0:
        ...
        start_IGR_seq = base_record.seq[:first_gene_left]
        start_IGR = make_IGR_feature(0, first_gene_left)
        start_IGR.qualifiers["locus_tag"] = "start_IGR_" + first_gene_name
    ...
    if gene_list[-1][1] < len(base_record.seq):
        ...
        end_IGR_seq = base_record.seq[last_gene_right:]
        end_IGR = make_IGR_feature(last_gene_right, len(base_record.seq))
        end_IGR.qualifiers["locus_tag"] = last_gene_name + "_IGR_end"

    # Since the start and end IGRs are actually one continuous IGR for a circular genome,
    # if together they meet the length criterion, append them to the genbank record.
    if len(end_IGR_seq) + len(start_IGR_seq) >= min_igr_length:
        base_record.features.append(start_IGR)
        base_record.features.append(end_IGR)
        igr_count += 2
